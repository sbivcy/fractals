...
def colormap_gen():
    ...
    for a in range(1, 256):
        colormap.append((0, int(255 * (log(a)/log(255))), 255))
    for a in range(1, 256):
...
def is_in_set(real, imaginary, iteration_count, resolution, frame_size, offset, colormap, c):
...
def main(iteration_count, resolution, frame_size, offset, julia, frame):
    ...
    for real in range(0, resolution):
        for imaginary in range(0, resolution):
            img[imaginary, real] = is_in_set(real, imaginary, iteration_count, resolution, frame_size, offset,
                                             colormap, julia)
        print(f"\r{format(real / resolution, '.2%')}", end="")
    print_end_info(start, time(), resolution, iteration_count)
    Image.fromarray(img, "RGB").save(f"frames/img-{'0' * (3-len(str(frame)))}{int(frame)}.png")
